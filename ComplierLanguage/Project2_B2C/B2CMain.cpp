...
map<string, string> arg_symbolTable;

map<string, string> func_TypeTable;

map<string, map<string,string>> function_map ; //^ function 이름 : ( arg 및 변수 : 타입)
map<string, map<string,string>> function_agrv_map ;//^ function 이름 :( arg : 타입)
map<string, vector<string>> function_agrc_map ; //^ `` argv 1234 순서대로
map<string, string> arg_in_func ; //^ ( arg : 타입 )
vector<string> argc_in_func ;


string assn_varName ;
string assn_varType;
string assn_funcName ;
string assn_arg ;
string functionName;
    ...
    any visitReturnstmt(BParser::ReturnstmtContext *ctx) override {
        if (ctx->expression()) {
            ...
            if(func_type_incompat){
                ...
                cout << "/*[Return Type Error] Line " << lineNum <<": In int main() Function > return value : " << return_var << " ( type : " << return_type
                    <<" ) imcompatible ! */" << endl;
            }
    ...
    any visitExpr(BParser::ExprContext *ctx) override {
        ...
        // binary operator
            ...
            switch(incompat_t_err){
                case 1 :
                    cout << " /*[Incompatible Type Error] Line " << lineNum <<":" << name << " : "<< left<<"("<< arg_in_func[left] << ") != "
                ...
                case 2 :
                    ...
                    break;
                case 3 :
                    cout << " /*[Incompatible Type Error] Line " << lineNum <<":" << name << " : "<< left<<"("<< determineType(left) << ") != "
                            << right << "("<< arg_in_func[right] << ")*/" << endl ;
                    break;
                case 4 :
                    cout << " /*[Incompatible Type Error] Line " << lineNum <<":" << name << " : "<< left<<"("<< determineType(left) << ") != "
        ...
        else if (ctx -> constant())
        {
            ...
            if(arg_in_func.find(assn_varName)!=arg_in_func.end()){
                ...
                if((arg_in_func[assn_varName]).compare(inferredType) && (arg_in_func[assn_varName].compare("auto")!=0)
                    && var_assign_flag) {
                    int lineNum = ctx->getStart()->getLine();
                    cout << "/* [Redifination Error]  Line " << lineNum <<": Redifining "<< assn_varName << "(type: "<< arg_in_func[assn_varName] <<")"
            ...
            if(var_assign_flag){
                ...
                string right_Type;
                int right_flag = 0;
                right_name = getFunctionName(right_name);

                if(checkTable(tmp_Table,right_name)==1){
                    right_Type = tmp_Table[right_name];
                    right_flag = 1; 
                }
                else if(checkTable(func_TypeTable,right_name)==1){
                    right_Type = func_TypeTable[right_name];
                    right_flag = 1;
                }
